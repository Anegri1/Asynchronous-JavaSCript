 ...
 */
const add = async (...values) => { return values.reduce((prev, current) => prev + current, 0)
 ...
 * @returns {Promise<Number>}
 */
const subtract = async (...values) => { return values.reduce((prev, current) => prev + (Math.abs(current) * -1), 0)
